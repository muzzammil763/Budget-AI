...
- Microphone recordings use 16 kHz mono PCM16 WAV and are sent with matching recognition metadata through the authenticated `google-cloud-speech` Edge Function to Google Cloud Speech-to-Text. Google Text-to-Speech MP3 output is played from short-lived `.mp3` files for reliable Apple-platform decoding. The Google credential stays in Supabase secrets and is never bundled into Flutter.

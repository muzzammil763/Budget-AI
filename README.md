...
Finance Insights includes top categories and full category breakdowns for both income and expenses. Past-month Net becomes Saved or Overused, with no zero-result metric. A saved balance or deficit is attributed to the following month: a month with 20,000 income, 17,000 expenses, and 3,000 saved displays 17,000 adjusted income there and carries 3,000 into the next month. Overall totals still count the money once. Existing carry rows are recalculated from the earliest month at startup, so late edits or synced entries repair all later month balances. In Overall, historical income and expense remain activity totals while Net shows the current carried balance available now. Each nonempty month has a **Monthly Summary** card at the bottom. Generate requests a short OpenAI summary using freshly loaded monthly totals, category breakdowns, and daily spending; Regenerate replaces the saved result only after a successful request. Opening the card reads the saved summary without an AI call.

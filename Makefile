...
apk:
	...
	@flutter build apk --release --target-platform android-arm64 --split-debug-info=build/debug-info --obfuscate; status=$$?; \
...
apk-split:
	...
	@printf 'storeFile=%s\nstorePassword=%s\nkeyAlias=%s\nkeyPassword=%s\n' \
		"$(KEYSTORE_FILE)" "$(STORE_PASSWORD)" "$(KEY_ALIAS)" "$(KEY_PASSWORD)" \
		> android/key.properties
	@flutter build apk --release --split-per-abi --split-debug-info=build/debug-info --obfuscate; status=$$?; \
	rm -f android/key.properties; \
	exit $$status
	@version=$$(awk '/^version:[[:space:]]*/ { print $$2; exit }' pubspec.yaml); \
	dest="$(DOCUMENTS_DIR)"; \
...
aab:
	...
	@printf 'storeFile=%s\nstorePassword=%s\nkeyAlias=%s\nkeyPassword=%s\n' \
		"$(KEYSTORE_FILE)" "$(STORE_PASSWORD)" "$(KEY_ALIAS)" "$(KEY_PASSWORD)" \
		> android/key.properties
	@flutter build appbundle --release --split-debug-info=build/debug-info --obfuscate; status=$$?; \
	rm -f android/key.properties; \
	exit $$status
	@version=$$(awk '/^version:[[:space:]]*/ { print $$2; exit }' pubspec.yaml); \
	dest="$(DOCUMENTS_DIR)"; \

...
lib/
  ...
  src/
    ...
    banking/                    Plaid multi-bank connection and import surface
      connected_banks_screen.dart  Connections, accounts, combined view, sync history
      bank_connection_service.dart Plaid Link + authenticated Edge Function client
      bank_transaction_mapper.dart Provider mapping and override-preserving reconciliation
      bank_models.dart          Connection, account, and sync-history models
    settings/
      ...
      local_speech_models_screen.dart   Download/select/remove Whisper models
      permission_preferences_service.dart  Soft on/off for notifications + background
    speech/                     Sherpa-ONNX Whisper Small speech-to-text
    sync/
...
- `LocalSettingsStore` migrates legacy Shared Preferences into SQLite. Account display name, currency, and message bubble style synchronize through `user_settings` via `AccountSettingsSyncService`; custom bubble definitions use the `custom_bubble_styles` JSONB column while `bubble_style` stores the selected preset or `custom:<id>`, so colors, shape, and pattern travel with the account. Onboarding and notification/background choices stay local; downloaded speech-model files also remain device-only. Permission grants made during onboarding enable the matching local soft preference, and authentication/account sync never replaces it.
...
- `LocalSpeechService` uses Sherpa-ONNX for on-device Whisper transcription. `LocalSpeechModelManager` downloads and removes the Whisper Small English archive from application support storage; there is no model-selection UI. The model exposes detailed engine, storage, and trade-off information. Voice is input-only: chat replies are never synthesized or played aloud.
- Optional Plaid banking uses `plaid_flutter` only for Link. Authenticated
  `plaid-banking` Edge Function actions create Link tokens, exchange public
  tokens, list connections, cursor-sync Transactions, and disconnect Items;
  the public `plaid-webhook` function verifies Plaid's ES256 signature and raw
  body hash before marking a connection for sync. Multiple institutions and
  accounts are supported. Plaid access tokens are AES-256-GCM encrypted under
  the server-only `PLAID_TOKEN_ENCRYPTION_KEY` in RLS-protected,
  service-role-only tables and
  never reach Flutter. Transaction plaintext is not retained by the backend:
  Flutter maps it into local entries, after which existing account encryption
  uploads only ciphertext. Consequently sync completes while the app is open,
  not as a server-side plaintext background import.
...
- **Budget Hub** — sectioned landing screen with bento-style Finances and Insights Quick Actions, inline Account controls for editing the name, viewing the read-only email, and password recovery, Preferences for Currency display, Offline Speech Model and Message bubble, plus Notifications, Android Background Service, and Replay onboarding controls under App Behavior. Replay onboarding pushes the same five-page flow with a top Back action; Back and final completion both return to Budget Hub instead of replacing the route with auth. Sign out and permanent account deletion sit at the end in a dedicated Danger Zone; deletion runs through an authenticated Edge Function and account-owned cloud rows cascade with the auth user. The currency picker is also available during onboarding before sign-in; preset selection and custom currency creation persist locally for the later account flow. It uses a responsive search field with an adjacent circular add action that opens a dedicated custom-currency add/edit form; custom displays are limited to five characters and saved custom entries expose edit/delete actions. It reveals the selected display on entry, jumps directly to newly saved custom displays without a visible scroll animation, and remains open after selection until the user navigates back. The bubble-style picker currently exposes search without a custom-add action; its second preset is Outline, using the Primary shape with a transparent fill and primary-color border. Existing custom styles retain edit/delete actions, and its custom editor supports named bubble/text/pattern colors, rounded/chat/pill/angular/ticket shapes, none/dots/diagonal/grid/waves patterns, and a top-floating live preview that remains visible while controls scroll. Offline Speech Model exposes only Whisper Small English with multi-line technical details and no selection controls. Monthly AI usage lives in the chat top chrome rather than Budget Hub. Bubble-style, currency, and speech-model pickers share one aesthetic (12-radius cards and `outline@0.25`/primary borders). Encryption is not a setting — it is handled by the mandatory gate.
- **Connected Banks** — optional Plaid Link entry under Budget Hub for multiple
  supported institutions and accounts. Connection setup asks for the bank
  country and an initial history range, then blocks interaction with a blurred
  full-screen spinner only until the Plaid sheet is presented; the dashboard offers per-account and
  combined bank activity, explicit sync, and the last 30 sync summaries.
  Imported bank amounts, dates, type, and pending/posted state are provider
  controlled and read-only. Users can edit the displayed note/category or
  exclude a record from budgets. Reconciliation handles added, modified,
  removed, and pending-to-posted transactions without duplicating entries;
  excluded records remain visible but do not affect Finances totals or Insights.

...
lib/
  ...
  src/
    ...
    chat/                       Chat UI + OpenAI Responses streaming + tool loop
      ...
      model_picker_screen.dart  Full-screen model picker (was a sheet)
    ...
    settings/
      settings_screen.dart      Name, Finances, Insights, Currency, Model, Speech,
      ...
      model_settings_service.dart, user_name_settings_service.dart
    ...
    sync/
      ...
      account_settings_sync_service.dart   Syncs user_settings (name/model/currency/bubble)
...
- `LocalSettingsStore` migrates legacy Shared Preferences into SQLite. Account display name, OpenAI model, currency, and message bubble style synchronize through `user_settings` via `AccountSettingsSyncService`; onboarding and speech-model selections stay local.
...
- The model catalog lives in `lib/src/chat/ai_models.dart`; the default is `gpt-5.6-luna`. GPT-5 chat requests use low reasoning effort and low text verbosity; prompts preserve important facts while removing repetition.
...
- **Chat** — streaming OpenAI chat that logs/queries finances via tools. Speech playback is scoped to microphone-originated turns; typed messages never trigger audio. The composer has one contextual action: Send when text exists, hold-to-talk when empty, Stop during a response. Tap the app-bar model name to open the model picker. Chat history is local-only. All styles use the bundled Google Sans font; Boldonse branding and monospaced code are preserved.
...
- **Settings** — inline name editor, Finances, Finance Insights, Currency display, OpenAI model, Offline Speech Models, Message bubble (dedicated screen), Notifications toggle, Android Background Service toggle, and Sign out. Bubble-style, model, currency, and speech-model pickers share one aesthetic (12-radius cards, `outline@0.25`/primary borders, SafeArea + max-width). Encryption is not a settings toggle — it is handled by the mandatory gate.

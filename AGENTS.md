...
Generated images use a 1:1 result slot with 12-pixel response-matching side insets. Once image generation starts, a square shimmer occupies the same slot until the image arrives. Chart prompts request a polished, readable square composition and reserve the bottom-right 32% × 12% for a local Stack overlay containing the same painted BudgetMarkIcon used by the chat empty state and small Boldonse Budget AI name. The generated-image viewer offers zoom and an AppBar Share action exporting a 1024×1024 PNG with that same branding, using the native share sheet and an iPad popover anchor. Source images remain local-only; sharing occurs only on user action.

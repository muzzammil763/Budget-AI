...
- **Budget Hub** — sectioned landing screen with bento-style Finances and Insights Quick Actions, an Account entry, Preferences for Currency display, Offline Speech Models and Message bubble, plus Notifications and Android Background Service controls under App Behavior. The dedicated Account screen contains the name editor, account identity, secure Supabase password-recovery action, and sign out. The currency picker uses a floating add action to open a dedicated custom-currency add/edit form; custom displays are limited to five characters and saved custom entries expose edit actions. Monthly AI usage lives in the chat top chrome rather than Budget Hub. Bubble-style, currency, and speech-model pickers share one aesthetic (12-radius cards and `outline@0.25`/primary borders). Encryption is not a setting — it is handled by the mandatory gate.

...
mandatory end-to-end encrypted Supabase sync, OpenAI STT/ElevenLabs TTS, and native iOS
...
lib/
  ...
  src/
    ...
    speech/
      openai_speech_service.dart  OpenAI STT client + ElevenLabs TTS playback/cache
    sync/
...
speech uses an authenticated Supabase proxy to OpenAI and ElevenLabs.
...
- `OpenAiSpeechService` sends 44.1 kHz mono PCM16 WAV recordings to the authenticated `openai-speech` Edge Function, which uses `gpt-4o-mini-transcribe`, a short English/Urdu/Roman-Urdu finance-command prompt, and the existing server-only `OPENAI_API_KEY`. Empty transcripts return a retry hint instead of a raw exception. Reply synthesis uses ElevenLabs `eleven_multilingual_v2`, free-compatible premade voice `JBFqnCBsd6RMkjVDRZzb`, and server-only `ELEVENLABS_API_KEY`. Generated MP3 chunks are cached in temporary app storage to avoid repeat synthesis charges while cached. Microphone-originated responses auto-play in the foreground after the final typewriter reveal; typed-chat responses play/stop only when the user taps a fully completed assistant response. Playback taps are disabled throughout loading, streaming, reconnects, and tool execution. Startup deletes the retired `speech_models` application-support directory and its selection key.
...
- **Chat** — streaming OpenAI chat that logs/queries finances via tools. Tool calls are visible inline in chronological order: single calls show expandable status rows, consecutive calls use expandable grouped summaries, and details expose arguments/results. Successful expense/income adds also render a separate English visual card with the saved title, amount, category, date, and time. Finance-list results append after the completed assistant prose through the existing responsive Markdown table view, with date, time, entry, amount, category, and type. These visuals are excluded from speech. Hold-to-talk uses OpenAI mini transcription; its completed reply auto-plays through ElevenLabs TTS. Typed-chat replies remain silent until their fully completed assistant response is tapped, and tapping again stops playback. Assistant prose spells currency amounts and names in the response language (including English, Urdu script, and Roman Urdu); for list queries it gives the natural result and mentions the visual table. Speech omits Markdown table cells and reads the response's short same-language table introduction instead. The top-right chrome contains equal-size New Chat, Finances (Budget mark), and Budget Hub actions. Entering Chat preloads monthly AI usage, the account's admin role, and the authorized admin user list; Settings opens immediately from cached state while refreshes continue in the background. The normal composer has no prefix icon and keeps one contextual trailing action: Send when text exists, hold-to-talk when empty, Stop during a response. The recording pulse still occupies the leading composer slot while recording. Completed-response notifications convert the complete Markdown response to readable plain text and use Android's expandable large-text style; the OS controls collapsed and lock-screen visibility. Leaving the foreground during an active response skips the buffered typewriter reveal, requests the limited iOS background-execution window, and schedules notification delivery before releasing it; iOS can still suspend unusually long work. Markdown tables left-align every header and value column and retain compact numeric currency displays while prose uses spoken number words. The model is not user-facing anywhere in the UI. Chat history is local-only. All styles use the bundled Google Sans font; Boldonse branding and monospaced code are preserved.

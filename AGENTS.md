...
mandatory end-to-end encrypted Supabase sync, Google Cloud speech, and native iOS
...
lib/
  ...
  src/
    ...
    speech/
      google_cloud_speech_service.dart  Cloud STT client, TTS playback/chunking
    sync/
...
speech uses an authenticated Supabase proxy to Google Cloud.
...
- `GoogleCloudSpeechService` sends locale-aware PCM16 WAV recordings to the authenticated `google-cloud-speech` Edge Function for Google Cloud Speech-to-Text. It reads the finalized WAV header so recognition metadata matches any sample-rate/channel adjustment made by the device, and the proxy uses Google's `command_and_search` short-utterance model. Speech synthesis uses preview `gemini-3.1-flash-tts-preview`, the `Zubenelgenubi` voice, and a warm/welcoming style prompt. Chunked MP3 output plays from short-lived `.mp3` files so AVPlayer can identify it reliably. `GOOGLE_CLOUD_API_KEY` exists only as a Supabase secret. Startup deletes the retired `speech_models` application-support directory and its selection key. Microphone-originated responses auto-play in the foreground after the final typewriter reveal; typed-chat responses play/stop only when the user taps a fully completed assistant response. Playback taps are disabled throughout loading, streaming, reconnects, and tool execution.
...
- Never claim a lost recovery key can be recovered. Chat history must remain excluded from Supabase synchronization. Never expose the Google Cloud speech credential in Flutter.

...
mandatory end-to-end encrypted Supabase sync, OpenAI STT/device TTS, and native iOS
...
lib/
  ...
  src/
    ...
    speech/
      openai_speech_service.dart  OpenAI STT client + device TTS playback
    sync/
...
speech uses an authenticated Supabase proxy to OpenAI plus device-native TTS.
...
- `OpenAiSpeechService` sends PCM16 WAV recordings to the authenticated `openai-speech` Edge Function, which uses `gpt-4o-mini-transcribe` and the existing server-only `OPENAI_API_KEY`. Reply playback uses `flutter_tts` with installed system voices: Urdu script prefers `ur-PK`, Roman Urdu prefers English, and missing locales fall back to an installed voice. Microphone-originated responses auto-play in the foreground after the final typewriter reveal; typed-chat responses play/stop only when the user taps a fully completed assistant response. Playback taps are disabled throughout loading, streaming, reconnects, and tool execution. Startup deletes the retired `speech_models` application-support directory and its selection key.

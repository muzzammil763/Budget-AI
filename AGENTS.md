...
lib/
  ...
  src/
    ...
    settings/
      ...
      ai_response_settings_service.dart  Device-local OpenAI Fast mode choice
...
- Image input uses `image_picker` for camera/gallery, at most three attachments per message, normalized to PNG (up to a 1600-pixel longest edge and iteratively reduced until at most 4 MB). `ChatMessage.images` and generated `image` blocks persist in local SQLite, never account sync. `chat_image_widgets.dart` owns image normalization, previews, and zoom viewing. Provider context retains only the latest three image inputs while local history keeps older previews. Structured `input_image` parts and active `image_generation_call` items survive tool replay. The active chat model receives the hosted `gpt-image-2` tool only for explicit visual-creation prompts; those requests include only the finance list/summary tools needed to prepare a chart, keeping the proxy tool payload bounded. `openai-responses/request_validation.ts` bounds image inputs and fixes image generation to medium quality, 1024×1024, one hosted call per request. Image tool charges are additional to text usage. All media passes through authenticated proxies; no public storage uploads.
...
- `finance_summary` retains carry-in for a single calendar month and excludes internal transfers from multi-month totals and category breakdowns, preventing saved money or deficits from being counted twice.
...
- **Chat** — streaming OpenAI chat that logs/queries finances via tools. Tool calls are visible inline in chronological order: single calls show expandable status rows, consecutive calls use expandable grouped summaries, and details expose arguments/results. Hold-to-talk uses OpenAI transcription; all chat replies stay silent. Camera/gallery inputs can be analyzed as receipts, expenses, or income; finance tools log requested entries, with clarification for unreadable details and no double-counting receipt totals. Requested budget/chart images appear as zoomable generated images. The top-right chrome contains equal-size New Chat, Finances (Budget mark), and Budget Hub actions. Entering Chat preloads monthly AI usage, the account's admin role, and the authorized admin user list; Settings opens immediately from cached state while refreshes continue in the background. The normal composer has a + prefix opening a blurred rounded Camera/Photos panel with the same animated border, up to three removable and zoomable previews, and up to four text lines without a separate expanded editor. Its inactive empty/single-line state is fully pill-shaped; wrapped text or attachments use 28-radius corners and move actions below the text. Its contextual trailing action is Send when text or images exist, hold-to-talk when empty, and Stop during a response. The recording pulse still occupies the leading composer slot while recording. Completed-response notifications convert the complete Markdown response to readable plain text and use Android's expandable large-text style; the OS controls collapsed and lock-screen visibility. Leaving the foreground during an active response skips the buffered typewriter reveal, requests the limited iOS background-execution window, and schedules notification delivery before releasing it; iOS can still suspend unusually long work. Markdown tables left-align every header and value column. The model is not user-facing anywhere in the UI. Chat history is local-only. All styles use the bundled Google Sans font; Boldonse branding and monospaced code are preserved.
- **Finances** — month/overall pill scope, balance card without a separate balance heading: current amounts stand alone, past nonzero amounts end in Saved/Overused using the same 28-point typography; past zero balances omit that row. Confirmed rollovers say they were transferred to the next month as Income/Expense. Overall income/expense totals count money once. Monthly activity moves an outgoing saved balance or deficit out of its source month's income/expense and includes it as the next month's carry-in (for example, 20,000 income minus a 3,000 saving displays as 17,000 source-month income and 3,000 in the next month); the Saved/Overused outcome still uses the source month's actual closing balance. The screen has a keyboard-responsive keyword-search row with a circular manual-add button. Tapping an entry opens the shared create/edit form directly (no details sheet, no swipe gestures). Save is available in the AppBar and body; existing entries expose body-level delete backed by a confirmation sheet.
- **Insights** — opens scoped to the selected/current month (not Overall). Past-month Net metrics become Saved/Overused and disappear at zero. Income and expense categories each have top-category and full-breakdown cards. Overall reporting excludes savings and deficit transfers from historical income/expense activity; the Overall card's Net uses the current carried month balance so it represents the amount actually available now. Individual months retain their carryovers. Each nonempty month ends with an on-demand Monthly Summary card, including Generate/Regenerate and a locally saved result. Generation reloads fresh monthly data and sends compact totals/categories/daily expenses to the existing authenticated OpenAI utility endpoint with the active model, requesting 2–3 sentences. Summaries use account/month keys in Shared Preferences, never sync, and clear on account exit; in-flight requests cannot restore data after account exit. The spending heatmap is clipped to the first tracked entry (header reads "Since <Month Year>" until a full year accrues). The 30-day and month activity charts are numberless bars sized by expense; tap or hold a bar for a transient popup with that day's total and date.
